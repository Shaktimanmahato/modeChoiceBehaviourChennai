...
actual_df <- actual_df %>%
  mutate(
    ...
    # Additional variables for utility specification
    is_secondary = 0
  )
...
# ======================================================
# 2. FOUR CASES FOR SIGMA AND BETA EQUALITY TESTS
# FOR PRIMARY vs SECONDARY MODE CHOICE IN MULTIMODAL TRAVELERS
# ======================================================
...
apollo_control <- list(
  ...
  panelData = TRUE  # Added panelData = TRUE
)
...
apollo_probabilities_case1 <- function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  V[["tw"]] = asc_tw + b_tt * twlrtt1_mar20 + b_cost * carco2_mar20 +
    b_tw_reliability * (TWRELM + TWRELH) * twhh + b_pickup * PickDrop
  
  V[["car"]] = asc_car + b_tt * cartt1_mar20 + b_cost * carco2_mar20 + b_pickup * PickDrop
  
  V[["bus"]] = asc_bus + b_tt * bustt1_mar20 + b_cost * busco2_mar20 +
    b_scaptive * scaptive_mar20 + b_income * inc40_more + b_los * BEASEGET
  
  V[["trn"]] = asc_trn + b_tt * tratt1_mar20 + b_cost * traco2_mar20 +
    b_scaptive * scaptive_mar20 + b_income * inc40_more + b_los * TEASEGET
  
  V[["ipt1"]] = asc_ipt1 + b_cost * autoco2_mar20 + b_female * female
  
  V[["ipt2"]] = asc_ipt2 + b_scaptive * scaptive_mar20 + b_female * female + b_oddtime * oddtim
  
  V[["nmt"]] = asc_nmt + b_tt * nmttt1 + b_scaptive * scaptive_mar20 + b_oddtime * oddtim
  ...
  P <- apollo_panelProd(P, apollo_inputs, functionality)  # Added this line
...
apollo_probabilities_case2 <- function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  V[["tw"]] = asc_tw + d_asc_tw*is_secondary + 
    ...
    (b_cost + d_b_cost*is_secondary)*carco2_mar20 +
    (b_tw_reliability + d_b_tw_reliability*is_secondary)*(TWRELM + TWRELH)*twhh + 
    (b_pickup + d_b_pickup*is_secondary)*PickDrop
  ...
  V[["bus"]] = asc_bus + d_asc_bus*is_secondary + 
    ...
    (b_cost + d_b_cost*is_secondary)*busco2_mar20 + 
    (b_scaptive + d_b_scaptive*is_secondary)*scaptive_mar20 + 
    (b_income + d_b_income*is_secondary)*inc40_more + 
    (b_los + d_b_los*is_secondary)*BEASEGET
  
  V[["trn"]] = asc_trn + d_asc_trn*is_secondary + 
    (b_tt + d_b_tt*is_secondary)*tratt1_mar20 + 
    (b_cost + d_b_cost*is_secondary)*traco2_mar20 + 
    (b_scaptive + d_b_scaptive*is_secondary)*scaptive_mar20 + 
    (b_income + d_b_income*is_secondary)*inc40_more + 
    (b_los + d_b_los*is_secondary)*TEASEGET
  
  V[["ipt1"]] = asc_ipt1 + d_asc_ipt1*is_secondary + 
    (b_cost + d_b_cost*is_secondary)*autoco2_mar20 + 
    (b_female + d_b_female*is_secondary)*female
  
  V[["ipt2"]] = asc_ipt2 + d_asc_ipt2*is_secondary + 
    (b_scaptive + d_b_scaptive*is_secondary)*scaptive_mar20 + 
    (b_female + d_b_female*is_secondary)*female + 
    (b_oddtime + d_b_oddtime*is_secondary)*oddtim
...
apollo_probabilities_case3 <- function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  V[["tw"]] = (asc_tw + b_tt * twlrtt1_mar20 + b_cost * carco2_mar20 +
                 b_tw_reliability * (TWRELM + TWRELH) * twhh + b_pickup * PickDrop) * scale_factor
  
  V[["car"]] = (asc_car + b_tt * cartt1_mar20 + b_cost * carco2_mar20 + b_pickup * PickDrop) * scale_factor
  
  V[["bus"]] = (asc_bus + b_tt * bustt1_mar20 + b_cost * busco2_mar20 +
                  b_scaptive * scaptive_mar20 + b_income * inc40_more + b_los * BEASEGET) * scale_factor
  
  V[["trn"]] = (asc_trn + b_tt * tratt1_mar20 + b_cost * traco2_mar20 +
                  b_scaptive * scaptive_mar20 + b_income * inc40_more + b_los * TEASEGET) * scale_factor
  
  V[["ipt1"]] = (asc_ipt1 + b_cost * autoco2_mar20 + b_female * female) * scale_factor
  
  V[["ipt2"]] = (asc_ipt2 + b_scaptive * scaptive_mar20 + b_female * female + b_oddtime * oddtim) * scale_factor
  
  V[["nmt"]] = (asc_nmt + b_tt * nmttt1 + b_scaptive * scaptive_mar20 + b_oddtime * oddtim) * scale_factor
...
apollo_probabilities_case4 <- function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  V[["tw"]] = (asc_tw + d_asc_tw*is_secondary + 
                 ...
                 (b_cost + d_b_cost*is_secondary)*carco2_mar20 +
                 (b_tw_reliability + d_b_tw_reliability*is_secondary)*(TWRELM + TWRELH)*twhh + 
                 (b_pickup + d_b_pickup*is_secondary)*PickDrop) * scale_factor
  ...
  V[["bus"]] = (asc_bus + d_asc_bus*is_secondary + 
                  ...
                  (b_scaptive + d_b_scaptive*is_secondary)*scaptive_mar20 + 
                  (b_income + d_b_income*is_secondary)*inc40_more + 
                  (b_los + d_b_los*is_secondary)*BEASEGET) * scale_factor
  ...
  V[["trn"]] = (asc_trn + d_asc_trn*is_secondary + 
                  ...
                  (b_scaptive + d_b_scaptive*is_secondary)*scaptive_mar20 + 
                  (b_income + d_b_income*is_secondary)*inc40_more + 
                  (b_los + d_b_los*is_secondary)*TEASEGET) * scale_factor
  
  V[["ipt1"]] = (asc_ipt1 + d_asc_ipt1*is_secondary + 
                   (b_cost + d_b_cost*is_secondary)*autoco2_mar20 + 
                   (b_female + d_b_female*is_secondary)*female) * scale_factor
  
  V[["ipt2"]] = (asc_ipt2 + d_asc_ipt2*is_secondary + 
                   (b_scaptive + d_b_scaptive*is_secondary)*scaptive_mar20 + 
                   (b_female + d_b_female*is_secondary)*female + 
                   (b_oddtime + d_b_oddtime*is_secondary)*oddtim) * scale_factor
...
# ======================================================
# 3. MODEL COMPARISON AND OUTPUT GENERATION
# ======================================================
...
# ======================================================
# 4. OUTPUT FOR EACH CASE AS REQUESTED (CONTINUED)
# ======================================================

for (case_num in 1:4) {
  case_name <- paste0("Case", case_num)
  cat("\n", rep("=", 50), "\n", sep="")
  ...
  # Create case labels for filtering
  case_labels <- c(
    "Case1: σ=1, β=1",
    "Case2: σ=1, β≠1", 
    "Case3: σ≠1, β=1",
  ...
  primary_coef <- all_primary[all_primary$Case == case_labels[case_num], ]
  ...
  cat(rep("-", 42), "\n", sep="")
  secondary_coef <- all_secondary[all_secondary$Case == case_labels[case_num], ]
  print(secondary_coef[, c("Parameter", "Estimate", "t_value")])
  cat("\n")
}

# ======================================================
# 5. SUMMARY TABLE WITH ALL REQUESTED METRICS
# ======================================================
...
# ======================================================
# 6. LIKELIHOOD RATIO TESTS FOR HYPOTHESIS TESTING
# ======================================================
